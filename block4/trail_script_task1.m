...
for jj = 30: length(th)
    for ii = 1:length(th)
        current_out_dir = fullfile(out_dir,'Combined',['CCL',num2str(ii),'_',num2str(jj)]);
...
% region_out = all_reg;
% pix_out = all_pix;
save('CCL_COMBINE.mat','region_out','pix_out','th_corr','th_mae')
...
% For Sliding Window
for jj = 28: 29%length(th)
    for ii = 1:length(th)
        current_out_dir = fullfile(out_dir,'Combined',['SLW',num2str(ii),'_',num2str(jj)]);
        W4_task1(slw_masks,models_path,current_out_dir,3,[th(ii),th(jj)],plot_flag);
        %[region_out(ii,jj), pix_out(ii,jj)] = W3_Task4 (annotation_dir, current_out_dir,provided_masks_dir,current_out_dir);
%         region_out(ii,jj) = tmpr;
      % th_mae(ii,jj) = th(ii);
      % th_corr(ii,jj) = th(jj);
%         pix_out(ii,jj) = tmpp;
...
% region_out = all_reg;
% pix_out = all_pix;
save('SLW_COMBINE.mat','region_out','pix_out','th_mae','th_corr')
