...
function [ S_out ] = find_bBox( cur_mask,file_id,statistic_table,plot_flag,origin_im,main_idx ,threshold_sym)
if nargin <7
    threshold_sym = 0.72;
end
S_out = [];
...
if nargin < 3|| isempty(statistic_table)
    ...
    minmax_ratio = [0.4,1.45];
    minmax_mc_col = [0.42, 0.58];
    
    minmax_ff(1,:)   =   [0.42,0.62]; % Triangale
    minmax_ff(2,:)   =   [0.42,0.62]; % Triangale_Up
    minmax_ff(3,:)   =   [0.49,0.91]; % Circle
    minmax_ff(4,:)   =   [0.94,1.1]; % Rectangle
...
S_final = [];
type = '';
all_types = {'Triangle- Up', 'Triangle- Dn', 'Circle', 'Rectangle'};
method = 1;
if method ==1
    clr = hsv(length(S));
    for gg= 1: length(S)
        if FF(gg)>min_ff
            SmallBox = cur_mask(ceil(S(gg).BoundingBox(2)):ceil(S(gg).BoundingBox(2))+S(gg).BoundingBox(4)-1,ceil(S(gg).BoundingBox(1)):ceil(S(gg).BoundingBox(1))+S(gg).BoundingBox(3)-1);
            score_sym = symmetric_test( SmallBox );
            if score_sym>threshold_sym
                count = count+1;
                S_final = [S_final;S(gg)];
                
                temp_s = struct('x',S(gg).BoundingBox(1),'y',S(gg).BoundingBox(2),'w',S(gg).BoundingBox(3),'h',S(gg).BoundingBox(4));
                S_out = [S_out;temp_s];
                clr_final(count,:) = clr(count,:);
                type{count} = num2str(score_sym);
            end
        end
    end
    if plot_flag & ~isempty(S_final)
        txt_title = {[num2str(num2str(length(S_final))),' Connected Regions'];'Symmetric test' };
        plot_bBox(cur_mask,S_final,4,main_idx,txt_title,clr_final,type)
...
function plot_bBox(cur_mask,S,sub_idx,main_idx,txt_title,clr,type)
figure(main_idx);
subplot(2,2,sub_idx);
